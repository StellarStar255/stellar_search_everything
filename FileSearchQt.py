...
class FileSearchWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        # 必须挂菜单：GNOME/AppIndicator 的托盘项没有菜单就不显示图标。
        # 代价是 GNOME 上左键固定用于弹菜单（系统行为，应用无法改）；
        # 其它平台左键仍会触发 Trigger → 显示窗口
    ...
    def _on_tray_activated(self, reason):
        # 左键(Trigger,非 GNOME)/中键/双击 → 显示主窗口。
        # GNOME 左键被系统占用弹菜单，收不到 Trigger；用中键或“最小化到任务栏”兜底
